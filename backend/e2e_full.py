"""E2E test - output results to file."""
import sys, urllib.request, urllib.parse, json, os

sys.path.insert(0, '.')

ALL_KEYS = "ak_5ptli4fp16hlffsrv7mxqgmv7atry4g67sn0uj1ghdg7jom,ak_y05reahqfectpm7959dp9kj1a2n0sz3atu1qbrw5yt17y5g,ak_yqhvsksd061yhjtafzi0l4k4t134mnpz2slwvm45pkpwq0k,ak_w50lspzm38pmj2zdz8v9phbuvai1a1h0df1u0qh1gmx753p,ak_a8plc0spoptyh0o0ug3d4qjrwpxa05gl0qmnsfadpoq2x2r,ak_aaijzj1qg9l0165qb3achtc6gd44nuegre9jcp0wbzu2fim"

data = urllib.parse.urlencode({
    'job_title': 'assistente administrativo',
    'target_stack': 'administrativo',
    'seniority': 'Nenhum',
    'location': 'Nacional',
    'time_window': '30 dias',
    'negative_keywords': '',
    'jsearch_api_keys': ALL_KEYS
}).encode()

req = urllib.request.Request(
    'http://localhost:8000/api/market/analyze',
    data=data,
    headers={'Content-Type': 'application/x-www-form-urlencoded'}
)

out_path = r'C:\Users\xxxsa\OneDrive\Área de Trabalho\python\curriculo\backend\result_e2e.txt'
out_lines = []

def out(msg):
    out_lines.append(str(msg))
    print(msg)

try:
    out("Sending request...")
    with urllib.request.urlopen(req, timeout=600) as resp:
        raw = resp.read().decode()
        result = json.loads(raw)
        R = result.get('report', result)
        s = R.get('summary', {})
        out(f"Total jobs scanned: {s.get('total_jobs_scanned')}")
        out(f"Pre-filtered: {s.get('pre_filtered_count')}")
        out(f"Relevant analyzed: {s.get('relevant_jobs_analyzed')}")
        out(f"Discarded: {s.get('discarded_jobs')}")
        out(f"JSearch status: {s.get('jsearch_status')}")
        jm = s.get('jsearch_message', '')
        out(f"JSearch msg: {str(jm)[:200]}")

        stats = R.get('statistics', {})
        req_techs = stats.get('required_technologies', [])
        soft_skills = stats.get('top_soft_skills', [])
        langs = stats.get('top_languages', [])
        nice = stats.get('nice_to_have_skills', [])
        exp_stats = stats.get('experience_distribution', {})

        out(f"\n=== REQUIRED TECH (top 15) ===")
        for t in req_techs[:15]:
            out(f"  {t.get('skill')}: {t.get('count')} ({t.get('percentage',0)}%)")

        out(f"\n=== SOFT SKILLS (top 10) ===")
        for t in soft_skills[:10]:
            out(f"  {t.get('skill')}: {t.get('count')} ({t.get('percentage',0)}%)")

        out(f"\n=== LANGUAGES (top 10) ===")
        for t in langs[:10]:
            out(f"  {t.get('language')}: {t.get('count')} ({t.get('percentage',0)}%)")

        out(f"\n=== NICE TO HAVE (top 15) ===")
        for t in nice[:15]:
            out(f"  {t.get('skill')}: {t.get('count')} ({t.get('percentage',0)}%)")

        out(f"\n=== EXPERIENCE ===")
        out(f"  min: {exp_stats.get('min_years')}")
        out(f"  max: {exp_stats.get('max_years')}")
        out(f"  avg: {exp_stats.get('avg_years')}")
        out(f"  dist: {json.dumps(exp_stats.get('distribution', {}), ensure_ascii=False)}")

        out(f"\n=== REJECTION SAMPLES ===")
        reasons = s.get('rejected_reasons_sample', [])
        for r in reasons[:10]:
            out(f"  {r}")

        out(f"\n=== KEY JOBS (first 5 relevant) ===")
        jobs = R.get('jobs', [])
        for j in jobs[:5]:
            out(f"  Title: {j.get('title')}")
            out(f"    req: {json.dumps(j.get('requirements',[])[:5], ensure_ascii=False)}")
            out(f"    soft: {json.dumps(j.get('soft_skills',[])[:3], ensure_ascii=False)}")
            out(f"    nice: {json.dumps(j.get('nice_to_have',[])[:3], ensure_ascii=False)}")
            out(f"    lang: {j.get('languages',[])}")
            out(f"    exp: {j.get('experience_years')}")
            out(f"    status: {j.get('status')}")
            out()

        out(f"\nTotal jobs in report: {len(jobs)}")

except Exception as e:
    out(f"ERROR: {e}")
    import traceback
    out(traceback.format_exc())

with open(out_path, 'w', encoding='utf-8') as f:
    f.write('\n'.join(out_lines))
out(f"\n[Results saved to {out_path}]")
